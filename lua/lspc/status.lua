...
local function in_range(pos, range)
  ...
  if line == range.start.line and char < range.start.character or line == range['end'].line and
    char > range['end'].character then
    return false
  end
...
local function current_function_callback(_, _, result, _, _)
  ...
  local function_symbols = filter(extract_symbols(result), function(_, v)
  ...
  for i = #function_symbols, 1, -1 do
    ...
    if (sym.range and in_range(cursor_pos, sym.range)) or
      (_config.select_symbol and _config.select_symbol(cursor_pos, sym.raw_item)) then
...
local M = {update = update_current_function, _init = init}

M.on_attach = function(client)
  if client.resolved_capabilities.document_symbol then
    vim.api.nvim_command('au CursorHold <buffer> lua require"lsp_status".update()')
