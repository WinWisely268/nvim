...
if status_ok then
  ...
  -- ==
  local file_namer =
    function(_, _) return '%#StFileName#' .. el_builtin.file end
  ...
  local git_changes = function(_, buffer)
    ...
    if vim.api.nvim_buf_get_option(buffer.bufnr, 'bufhidden') ~= '' or buftype ==
      'nofile' or buftype == 'terminal' then return end
  ...
  local function diag_status()
    ...
    if buf_diagnostics.errors and buf_diagnostics.errors > 0 then
      table.insert(parts,
                   '%#LspDiagnosticsError#' .. icons.deviconTable['errors'] ..
                     buf_diagnostics.errors .. ' ')
    end
    if buf_diagnostics.warnings and buf_diagnostics.warnings > 0 then
      table.insert(parts,
                   '%#LspDiagnosticsWarning#' .. icons.deviconTable['warnings'] ..
  ...
  local set_status = function(_) return el_segments end
