...
local modes = {c = 'c', i = 'i', n = 'n', o = 'o', t = 't', v = 'v', x = 'x'}
    ...
    if type(rhs) == 'function' then
      ...
      if options.dot_repeat then
        ...
        rhs = function()
          ...
          vim.fn['repeat#set'](vim.api.replace_termcodes(key_mapping, true, true, true),
                               vim.v.count)
        end
...
function M.callerror(func, err)
  return string.format('Error (%s): %s', debug.getinfo(func).short_src, err)
end
